                ...
                <div>
                  <div class="reciates-from-des-class">
                    ...
                    <span class="spanright">
                      <el-input placeholder="请输入要分配的邦分数量" v-model="subdata.score_num" size="small" :disabled="true">
                  ...
                  <div class="reciates-from-des-class">
                    ...
                    <span class="spanright">
                      <i-select @on-change="test" placeholder="选择成员" v-model="subdata.department" :disabled="true">
              ...
              <el-col :span="12" :xs="24">
                <div>
                  ...
                  <div class="reciates-from-des-class">
                    ...
                    <span class="spanrightone">
                      <!-- <DatePicker  type="month" placeholder="请选择月份" ></DatePicker> -->
                      <el-input size="small" :disabled="true" v-model="subdata.effective_time" style="width: 195px"></el-input>
                    </span>
                  </div>
                </div>
              </el-col>
  ...
  export default {
    ...
    methods:{
      ...
      savereciatesing(){//保存
        ...
        params.append('effective_time',this.subdata.effective_time.getTime());//适用时间(适用月的时间戳)
      ...
      //获取赞赏数据
      getreciateses(){
        let url = window.localStorage.api+'/allotshow/appreciate?id='+this.viewid;
        this.$http.get(url).then(res=>{
          let item = res['data'].message;
          console.log(item)
          this.subdata.score_num=item.score;//分配数量
          this.subdata.department=item.department_id;//部门
          this.subdata.min_range=item.range_min;//互赠范围最小值
          this.subdata.max_range=item.range_max;//互赠范围最大值
          this.subdata.effective_time=this.timestampToTime(item.effective_time);//item.effective_time//适用时间
          this.subdata.praise_type=item.praise;//赞赏类型json对象
          this.subdata.isrank=item.isrank==1?true:false;//是否参与排名
          this.tagslist = item.praise;//赞赏类型json对象
        }).catch(err=>{
          this.$Message.error('网络错误请稍后再试');
        })
      },
      timestampToTime(timestamp) {//将时间搓转换为时间
